<template>
  <VContainer fluid>
        ...
        >
          <VCardText>
            <VLayout align-start>
              ...
              <VFlex v-if="!isItemActive(item)" xs10>
                <AssessmentItemPreview :item="item" :detailed="displayAnswersPreview" />
              </VFlex>
              ...
              <VLayout align-center class="toolbar">
                ...
                <VFlex
                  v-if="!isItemActive(item) && !isItemValid(item) && !isApplicationType(item)"
                  mr-2
                >
                  <template v-if="$vuetify.breakpoint.lgAndUp">
                    <Icon tabindex="0" aria-label="Incomplete" class="red--text"> error </Icon>
    ...
    <VBtn color="greyBackground" class="ml-0 mt-4" data-test="newQuestionBtn" @click="addItem">
...
export default {
  ...
  methods: {
    ...
    isApplicationType(itemData) {
      let applicationType = Object.keys(ActionTypeList);
      if (applicationType.includes(itemData.application_type)) {
        return true;
      }
    },
    isPerseusItem(item) {
